...
The project uses the AES-256 for encryption, `rand` for generating a random encryption key, and `base64` for encoding.
...
```bash
cargo run -- decrypt <encrypted_folder_path> <key>
```
